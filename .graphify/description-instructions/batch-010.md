...
- "supabase_auth_middleware_requiresupabaseauth": "requireSupabaseAuth" | kind=code-symbol | source=src/integrations/supabase/auth-middleware.ts:L9 | neighbors=[accounts.functions.ts, admin.functions.ts, blog.functions.ts, contact.functions.ts, monitoring.functions.ts, portal.functions.ts] | lang=en
...
- "scripts_publish_scheduled_posts": "publish-scheduled-posts.js" | kind=code-symbol | source=scripts/publish-scheduled-posts.js:L1 | neighbors=[6ccd4dc CMS, __dirname, loadEnv(), log(), LOG_DIR, LOG_FILE] | lang=en
